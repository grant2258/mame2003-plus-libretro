  ...
  for (i=0; i<MAX_BANKS; i++)
    if (banksize[i])
      ...
      case 8:
        ...
        break;
      case 16:
        state_save_register_UINT16("bank", i, "ram", (UINT16 *)(void *)cpu_bankbase[i], banksize[i]/2);
        break;
      case 32:
        state_save_register_UINT32("bank", i, "ram", (UINT32 *)(void *)cpu_bankbase[i], banksize[i]/4);
        break;
      }
...
{                                           \
  ...
  {                                         \
    read8_handler handler = (read8_handler)(FPTR)handlist[entry].handler;         \
...
{                                           \
  ...
  {                                         \
    ...
    read16_handler handler = (read16_handler)(FPTR)handlist[entry].handler;       \
...
{                                           \
  ...
  {                                         \
    ...
    read32_handler handler = (read32_handler)(FPTR)handlist[entry].handler;       \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        MEMREADEND(READ_WORD_UNALIGNED(&cpu_bankbase[entry][address])) \
    else \
    { \
        read16_handler handler = (read16_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        MEMREADEND(READ_WORD_UNALIGNED(&cpu_bankbase[entry][WORD_XOR_BE(address)])) \
    else \
    { \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        MEMREADEND(READ_WORD_UNALIGNED(&cpu_bankbase[entry][WORD_XOR_LE(address)])) \
    else \
    { \
        int shift = 8 * (address & 2); \
        read32_handler handler = (read32_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        MEMREADEND(READ_LONG_UNALIGNED(&cpu_bankbase[entry][address])) \
    else \
    { \
        read32_handler handler = (read32_handler)(FPTR)handlist[entry].handler; \
...
{                                           \
  ...
  {                                         \
    write8_handler handler = (write8_handler)(FPTR)handlist[entry].handler;       \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        cpu_bankbase[entry][BYTE_XOR_BE(address)] = data; \
    else \
    { \
        int shift = 8 * (~address & 1); \
        write16_handler handler = (write16_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        cpu_bankbase[entry][BYTE_XOR_LE(address)] = data; \
    else \
    { \
        int shift = 8 * (address & 1); \
        write16_handler handler = (write16_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        cpu_bankbase[entry][BYTE4_XOR_BE(address)] = data; \
    else \
    { \
        int shift = 8 * (~address & 3); \
        write32_handler handler = (write32_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        cpu_bankbase[entry][BYTE4_XOR_LE(address)] = data; \
    else \
    { \
        int shift = 8 * (address & 3); \
        write32_handler handler = (write32_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        WRITE_WORD_UNALIGNED(&cpu_bankbase[entry][address], data); \
    else \
    { \
        write16_handler handler = (write16_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        WRITE_WORD_UNALIGNED(&cpu_bankbase[entry][WORD_XOR_BE(address)], data); \
    else \
    { \
        int shift = 8 * (~address & 2); \
        write32_handler handler = (write32_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        WRITE_WORD_UNALIGNED(&cpu_bankbase[entry][WORD_XOR_LE(address)], data); \
    else \
    { \
        int shift = 8 * (address & 2); \
        write32_handler handler = (write32_handler)(FPTR)handlist[entry].handler; \
...
{ \
    ...
    address -= handlist[entry].offset; \
    if (entry <= STATIC_RAM) \
        WRITE_LONG_UNALIGNED(&cpu_bankbase[entry][address], data); \
    else \
    { \
        write32_handler handler = (write32_handler)(FPTR)handlist[entry].handler; \
...
static READ16_HANDLER( mrh16_bad )
{
  ...
  if (activecpu_address_bits() <= SPARSE_THRESH && unmap_value == 0) return READ_WORD_UNALIGNED(&cpu_bankbase[STATIC_RAM][offset * 2]);
  return unmap_value;
}
static READ32_HANDLER( mrh32_bad )
{
  ...
  if (activecpu_address_bits() <= SPARSE_THRESH && unmap_value == 0) return READ_LONG_UNALIGNED(&cpu_bankbase[STATIC_RAM][offset * 4]);
  return unmap_value;
}
...
static WRITE16_HANDLER( mwh16_bad )
{
  ...
  if (activecpu_address_bits() <= SPARSE_THRESH) COMBINE_DATA((data16_t *)(FPTR)&cpu_bankbase[STATIC_RAM][offset * 2]);
}
static WRITE32_HANDLER( mwh32_bad )
{
  ...
  if (activecpu_address_bits() <= SPARSE_THRESH) COMBINE_DATA((data32_t *)(FPTR)&cpu_bankbase[STATIC_RAM][offset * 4]);
}
...
static void verify_masks(void)
{
  ...
  for (i = 0;drivers[i];i++)
  {
    ...
    struct InternalMachineDriver driver_data;
    memset(&driver_data, 0, sizeof(driver_data));
    // Construct the driver machine configuration
    (*drivers[i]->drv)(&driver_data);
    ...
    if (romp)
    {
      for (cpunum = 0;cpunum < MAX_CPU;cpunum++)
      {
        if (driver_data.cpu[cpunum].cpu_type)
        {
          const struct Memory_ReadAddress *mra = driver_data.cpu[cpunum].memory_read;
          const struct Memory_WriteAddress *mwa = driver_data.cpu[cpunum].memory_write;
          const struct IO_ReadPort *iora = driver_data.cpu[cpunum].port_read;
          const struct IO_WritePort *iowa = driver_data.cpu[cpunum].port_write;
